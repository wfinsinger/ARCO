...
#  episodes with the CharAnalysis softwar, as described in Finsinger et al. (2014).      #
#                                                                                        #
...
#----------------------------------------------------------------------------------------#
#                                                                                        #
# The function requires five file path inputs (see below), as well as the following      #
...
# file as the corresponding function argumen. Details of the five files are below, and   #
# examples are included with this code:                                                  #
#                                                                                        #
#   RAW DATA                                                                             #
#     1. Seedle.csv = .csv table with charcoal-particle areas. Should have three columns #
#                     and as many rows as the number of observations:                    #
#                                                                                        #
...
#                                                                                        #
#     2. Smpl.csv   = .csv table with charcoal counts and charcoal areas. Should have    #
...
#     4. FireCsp.csv = CharAnalysis output table from analysis of CHARCOAL COUNTS and    #
#                      WITH minimum count test, i.e. with CharAnalysis parameter         #
#                      'minCountP' < 1.0 (e.g. 0.05 as in Higuera et al. [2009])         #
#     4. FireC.csv   = CharAnalysis output table from analysis of CHARCOAL COUNTS and    #
...
arco = function(Seedle, Smpl, FireA, FireCsp, FireC, 
...
# ----- LOAD files with Charcoal per sample and of Seedles
  Seedle <- read.csv("Seedle.csv", header=T, sep = ",") # file with charcoal-particle areas
  Smpl <- read.csv("Smpl.csv", header=T, sep = ",") # file with charcoal counts and charcoal areas

# ----- LOAD files with Fire history derived from CharAnalysis
  # CharAnalysis output, with Charcoal areas
    CA.dat <- read.csv("FireA.csv", header=T, sep = ",") 

  # CharAnalysis output, with Charcoal counts WITH pMinCount
    CC.dat <- read.csv("FireCsp.csv", header=T, sep = ",") 
  
  # CharAnalysis output, with Charcoal counts WITHOUT pMinCount
    CCno.dat <- read.csv("FireC.csv", header=T, sep = ",") 

# ----- Define minimum-area bootstrapping function 
  b.stat <- function(seedle.areas, sample.size, n.boot=1000, thresh.prob=0.95) { 
...
# -------------------------- RK stopped here for now...




# Defines peaks and non peaks in CharAnalysis record
...
for(i in 1:n.peak) {   #     i=1
  overthresh.ind[[i]] = peak.ind[i]
  ind.cur = peak.ind[i]-1
  while(CA.cpeak[ind.cur]>CA.thresh[ind.cur]) {
    overthresh.ind[[i]] = c(overthresh.ind[[i]], ind.cur)
    ind.cur = ind.cur-1
  }
...
underthresh.interval = matrix(NA, nrow=n.peak+1, ncol=2)
  ...
  nonpeak.Seedle = Seedle[ sapply(Seedle$Depth, function(x) {x %in% Smpl$Depth[unlist(nonpeaksamples.ind)]}), ]
  peak.Seedle = Seedle[ sapply(Seedle$Depth, function(x) {x %in% Smpl$Depth[unlist(peaksamples.ind)]}), ]



# RUNNING THE FUNCTION ####
# Set peak.samples having charcoal area > threshold pass test
# Sets peak.ind.screened = passed peak samples
# Sets peak.ind.notpass = peak samples that did not pass test
screen.pass = rep(0, n.peak)
area.thresh = rep(NA,nrow(Smpl))
for(i in 1:n.peak) {       #        i=j=1
...
# Prepares output file for comparison ####
# between simulated and real CharArea
...
write.csv(comp.perc, file = "comp_perc.csv", row.names = FALSE)
 

## Write new CharAnalysis output file that reflects peak screening ####
...
CA.colnames = c("cm Top_i (cm)","age Top_i_(yr BP)","char Count_i (#)","char Vol_i (cm3)","char Con_i (# cm-3)","char Acc_i (# cm-2 yr-1)","charBkg (# cm-2 yr-1)","char Peak (# cm-2 yr-1)","thresh 1 (# cm-2 yr-1)","thresh 2 (# cm-2 yr-1)","thresh 3 (# cm-2 yr-1)","thresh FinalPos (# cm-2 yr-1)","thresh FinalNeg (# cm-2 yr-1)","SNI (index) ","thresh GOF (p-val)","peaks 1 ","peaks 2 ","peaks 3 ","peaks Final","peaks Insig.","peak Mag (# cm-2 peak-1)","smPeak Frequ (peaks 1ka-1)","smFRIs (yr*fire-1)","nFRIs (#)","mFRI (yr fire-1)","mFRI_uCI (yr fire-1)","mFRI_lCI (yr fire-1)","WBLb (yr)","WBLb_uCI (yr)","WBLb_lCI (yr fire-1)","WBLc (unitless)","WBLc_uCI (unitless)","WBLc_lCI (unitless)")
  
  write.table(CA.dat.out, "FireA_screened.csv", col.names=CA.colnames, row.names=F, sep=",")


## Plots Seedle_area, C#/CA-ratio, and Sample Area vs Char_counts ####
pdf("1_Counts_Areas.pdf")
...
with (valAllSdl.perc, {
  pdf("2_Boxplots_all_Samples_Perc.pdf")
...
pdf("4_Comparison fire events_Perc.pdf")
par(mfrow=c(1,1))
...
with (valSdl.perc, {
  pdf("6_Boxplots_Screened_Samples_Perc.pdf")
  par(mai = c(1,1,0.5,0.5), cex.axis = 1.2, cex.lab = 1.3)
