...
#     the south-central Brooks Range, Alaska. Ecological Monographs 79:201–219.          #
#                                                                                        #
#----------------------------------------------------------------------------------------#
...
# ----- Find all samples contributing to peaks identified by CharAnalysis
  ...
  # Loop over all identified peaks to find all samples contributing to each peak. (Because when multiple samples in a row exceed the threshold, CharAnalysis marks only the lowermost as a peak.)
    for(i in 1:n.peak) {
      ...
      # Look for other samples belonging to this peak
...
with (valAllSdl.perc, {
  pdf(file.path(output.dir, "2_Boxplots_all_Samples.pdf"))
...
# Plots comparison between fire episodes #####
pdf(file.path(output.dir, "4_Comparison fire episodes.pdf"))
par(mfrow=c(1,1))
